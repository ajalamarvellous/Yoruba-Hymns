...
def extract_info(page: str, csv_writer: object):
    ...
    try:
        ...
        returned_links.append(next_link)
    except IndexError:
        logger.error("Next link out of index")  
    try:  
        prev_link = soup.select(".elementor-post-navigation__prev.elementor-post-navigation__link a")[0].get("href")
        returned_links.append(prev_link)
    except IndexError:
        logger.error("Previous link out of index")  
    logger.info(f"The new links to crawl {returned_links}")
...
def update_list(returned_links: list, parsed_pages: list, unparsed_pages: list):
    ...
    # check if the returned_links list is not empty
    if returned_links != []:
